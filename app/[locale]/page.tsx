...
function Hero() {
  ...
  const stats = [
    { value: "500+",     label: t("stats.instruments") },
    { value: "1:500",    label: t("stats.leverage") },
    { value: "0.0 pips", label: t("stats.spreads") },
    { value: "$10",      label: t("stats.minDeposit") },
  ];
  ...
  return (
    <section className="relative bg-[#020810] overflow-hidden" style={{ minHeight: "90vh" }}>
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none select-none"
        viewBox="0 0 1440 800" preserveAspectRatio="xMidYMid slice" fill="none" aria-hidden="true">
        <path d="M0,580 C120,545 240,510 360,475 C480,440 560,455 660,415 C760,375 840,355 940,315 C1040,275 1120,295 1220,255 C1320,215 1380,195 1440,175"
          stroke="#00CC44" strokeWidth="2" strokeOpacity="0.30" strokeLinecap="round" />
        <path d="M0,580 C120,545 240,510 360,475 C480,440 560,455 660,415 C760,375 840,355 940,315 C1040,275 1120,295 1220,255 C1320,215 1380,195 1440,175 L1440,800 L0,800 Z"
          fill="#00CC44" fillOpacity="0.05" />
        <path d="M0,640 C160,618 320,596 480,575 C640,554 760,540 920,522 C1080,504 1200,488 1440,468"
          stroke="white" strokeWidth="1" strokeOpacity="0.09" strokeLinecap="round" />
        <path d="M0,545 C100,518 210,508 320,475 C430,442 500,462 610,428 C720,394 810,374 920,340 C1030,306 1120,326 1230,292 C1340,258 1390,240 1440,222"
          stroke="#00CC44" strokeWidth="1" strokeOpacity="0.14" strokeLinecap="round" strokeDasharray="8 5" />
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 35% 100% at 0% 50%, rgba(2,8,16,0.65) 0%, transparent 100%),radial-gradient(ellipse 35% 100% at 100% 50%, rgba(2,8,16,0.65) 0%, transparent 100%),linear-gradient(to top, rgba(2,8,16,0.85) 0%, transparent 28%),linear-gradient(to bottom, rgba(2,8,16,0.55) 0%, transparent 22%)" }}
      />

      {/* Content */}
      <div className="relative z-10 flex flex-col items-center justify-center text-center w-full px-5 py-20 lg:py-28" style={{ minHeight: "90vh" }}>
        <div className="inline-flex items-center gap-2.5 mb-8">
          ...
          <span className="text-[11px] font-semibold text-white/35 uppercase tracking-[0.22em]">{t("badge")}</span>
        </div>

        <h1 className="font-extrabold text-white leading-[1.04] tracking-tight mb-7 max-w-5xl" style={{ fontSize: "clamp(44px, 6vw, 90px)" }}>
          {t("headline")}<br />
          <span className="text-[#00CC44]">{t("headline2")}</span>
        </h1>

        <p className="text-white/48 max-w-2xl leading-relaxed mb-11" style={{ fontSize: "clamp(17px, 1.5vw, 21px)" }}>
          {t("sub")}
        </p>
        ...
        <Link href="/company/promotions" className="text-[12px] text-white/22 hover:text-[#00CC44] transition-colors uppercase tracking-widest">
          {t("cta_promo")}
        </Link>
        ...
        <div className="mt-12 sm:mt-16 w-full max-w-lg sm:max-w-xl">
          <div className="grid grid-cols-4 border-t border-white/10 pt-6 sm:pt-8 gap-1 sm:gap-2">
            {stats.map((s, i) => (
              <div key={s.label} className={`flex flex-col items-center gap-1.5 ${i < stats.length - 1 ? "border-r border-white/10" : ""}`}>
                <span className="font-extrabold text-white leading-none text-[18px] sm:text-[22px] lg:text-[28px]">{s.value}</span>
