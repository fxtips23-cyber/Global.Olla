    ...
    <>
      ...
      <section className="py-16 bg-[#F5F7FA]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <div className="text-[11px] font-semibold text-[#29B5D4] uppercase tracking-widest mb-4">{t("categories_label")}</div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {categories.map(({ title, desc, links }, i) => {
              ...
              return (
                <div key={title} className="bg-white border border-gray-100 rounded-2xl p-5 flex flex-col hover:border-[#29B5D4]/20 hover:shadow-md transition-all">
                  ...
                  <div className="space-y-1 border-t border-gray-50 pt-3">
                    {links.map(([label, url]) => (
                      <Link key={label} href={url} className="flex items-center justify-between text-[11px] text-gray-500 hover:text-[#29B5D4] transition-colors py-0.5 group">
                        <span>{label}</span>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <div className="text-[11px] font-semibold text-[#29B5D4] uppercase tracking-widest mb-4">{t("process_label")}</div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {processSteps.map(([n, title, desc]) => (
              <div key={n} className="bg-[#F5F7FA] border border-gray-100 rounded-2xl p-5 text-center">
                <div className="text-2xl font-extrabold text-[#29B5D4] mb-3">{n}</div>
      ...
      <section className="py-16 bg-[#F5F7FA]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <div className="text-[11px] font-semibold text-[#29B5D4] uppercase tracking-widest mb-4">{t("contact_label")}</div>
          ...
          <div className="grid sm:grid-cols-2 gap-5 mb-6">
            {[
              ...
              <div key={title} className="bg-white border border-gray-100 rounded-2xl p-6">
                ...
                <Link href={href} className="w-full flex items-center justify-center border border-[#29B5D4]/25 hover:bg-[#29B5D4] hover:text-black hover:border-[#29B5D4] text-[#29B5D4] font-semibold py-2.5 rounded-xl text-[12px] transition-all">{cta}</Link>
              </div>
            ))}
          </div>
          <div className="text-center">
            <Link href="/contact-us" className="text-[13px] font-semibold text-[#29B5D4] hover:text-[#29B5D4] transition-colors underline underline-offset-4">{t("contact_full_link")}</Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-[#050C15]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <div className="text-[11px] font-semibold text-[#29B5D4] uppercase tracking-widest mb-4">{t("issues_label")}</div>
                ...
                <ol className="space-y-2">
                  {steps.map((s, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-[12px] text-white/45">
                      <span className="text-[10px] font-bold text-[#29B5D4] w-4 flex-shrink-0 mt-0.5">{i + 1}.</span>
                      {s}
                    </li>
                  ))}
                </ol>
      ...
      <section className="py-14 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-[11px] font-semibold text-[#29B5D4] uppercase tracking-widest mb-4 text-center">{t("times_label")}</div>
              ...
              <tbody className="divide-y divide-gray-50">
                {responseRows.map(([type, method, response, priority]) => (
                  <tr key={type} className="hover:bg-[#F9FAFB]">
                    ...
                    <td className="px-5 py-3 text-[#29B5D4] font-semibold text-[13px]">{response}</td>
                    <td className="px-5 py-3 text-[12px]">
                      <span className="px-2.5 py-0.5 rounded-full font-semibold"
                        style={{
                          background: priority === "High" || priority === "Alta" ? "rgba(41,181,212,0.1)" : priority === "Compliance" || priority === "Conformidade" ? "rgba(30,136,229,0.1)" : "rgba(107,114,128,0.1)",
                          color: priority === "High" || priority === "Alta" ? "#29B5D4" : priority === "Compliance" || priority === "Conformidade" ? "#1E88E5" : "#6B7280",
                        }}>
                        {priority}
                      </span>
                    </td>
