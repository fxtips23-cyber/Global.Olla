...
const promos = [
  ...
  {
    status: "Coming Soon",
    statusStyle: "bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800",
  ...
  },
  {
    ...
    statusStyle: "bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800",
    title: "Welcome Bonus",
    amount: "Up to $1,000",
    ...
    <>
      ...
      <section className="py-16 bg-[#F5F7FA] dark:bg-[#081018]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-extrabold text-[#111827] dark:text-[#F9FAFB] mb-3">Available Promotions</h2>
            <p className="text-gray-500 dark:text-[#9CA3AF] max-w-xl mx-auto text-[15px]">All promotions are subject to eligibility and Olla Trade terms. Please read all terms before participating.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-6">
            {promos.map((p) => (
              <div key={p.title} className={`relative bg-white dark:bg-[#0F1720] rounded-2xl flex flex-col overflow-hidden ${p.featured ? "border-2 border-[#00CC44]/35 shadow-xl shadow-[#00CC44]/8" : "border border-gray-100 dark:border-[#1F2937] shadow-sm"}`}>
                {/* Top accent */}
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <h3 className="text-xl font-extrabold text-[#111827] dark:text-[#F9FAFB] mt-1 mb-3">{p.title}</h3>
                  <p className="text-[13px] text-gray-500 dark:text-[#9CA3AF] leading-relaxed mb-5 flex-1">{p.desc}</p>

                  {/* Key terms */}
                  <div className="space-y-1.5 mb-5">
                    {p.terms.map((t) => (
                      <div key={t} className="flex items-start gap-2 text-[11px] text-gray-500 dark:text-[#9CA3AF]">
                        <IconCheck className="w-3.5 h-3.5 text-[#00CC44] flex-shrink-0 mt-0.5" />{t}
                      </div>
                    ))}
                  </div>

                  <Link href={p.href} className={`w-full flex items-center justify-center font-bold py-3 rounded-xl text-[13px] transition-all ${p.featured ? "bg-[#00CC44] hover:bg-[#00DD4A] text-black" : "bg-[#111827] dark:bg-[#1F2937] hover:bg-[#00CC44] text-white hover:text-black"}`}>
                    {p.cta}
                  </Link>
                </div>
              </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <h2 className="text-3xl font-extrabold text-[#111827] dark:text-[#F9FAFB] mb-5">Who Can Claim Promotions?</h2>
              <div className="space-y-3">
                {[
                  ...
                  <div key={t} className="flex items-start gap-3 bg-[#F5F7FA] dark:bg-[#0F1720] border border-gray-100 dark:border-[#1F2937] rounded-xl p-4">
                    ...
                    <div><div className="text-[13px] font-bold text-[#111827] dark:text-[#F9FAFB] mb-0.5">{t}</div><div className="text-[12px] text-gray-500 dark:text-[#9CA3AF] leading-relaxed">{d}</div></div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="py-12 bg-[#F5F7FA] dark:bg-[#081018]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="border border-amber-200 dark:border-amber-900/40 bg-amber-50 dark:bg-amber-900/10 rounded-2xl p-6">
            <div className="text-[13px] font-bold text-amber-900 dark:text-amber-300 mb-3">Important Promotion Notice</div>
            <div className="space-y-2 text-[12px] text-amber-800 dark:text-amber-200/60 leading-relaxed">
