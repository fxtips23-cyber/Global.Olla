    ...
    <section id="how-it-works" className="py-20 lg:py-28 bg-[#F8FAFC]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-8 relative">
          ...
          <div className="hidden md:block absolute top-14 left-1/3 right-1/3 h-0.5 bg-gradient-to-r from-[#00CC44]/30 via-[#00CC44] to-[#00CC44]/30 z-0" />

          {steps.map((step, index) => (
            <div key={step.number} className="relative flex flex-col items-center text-center group">
              <div className="relative z-10 w-28 h-28 rounded-full bg-white border-2 border-gray-200 flex flex-col items-center justify-center mb-6 shadow-lg group-hover:border-[#00CC44] group-hover:shadow-green-100 transition-all duration-300">
                <div className="text-[#00CC44] mb-1">{step.icon}</div>
                <div className="text-xs font-bold text-[#00AA38] tracking-widest">STEP {step.number}</div>
              </div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">{step.title}</h3>
              <p className="text-gray-500 text-sm leading-relaxed max-w-xs">{step.description}</p>

              {index < steps.length - 1 && (
                <div className="md:hidden mt-6 text-[#00CC44]">
        ...
        <div className="text-center mt-14">
          <Link
            ...
            className="inline-flex items-center gap-2 bg-[#00CC44] hover:bg-[#00DD4A] text-black font-bold px-8 py-4 rounded-md text-base transition-all duration-200 shadow-lg shadow-green-900/20 hover:-translate-y-0.5"
          >
