"use client";
import { useTranslations } from "next-intl";
import Link from "next/link";
import { IconBarChart, IconTrendingUp, IconLayers, IconActivity, IconGlobe, IconMonitor } from "../ui/Icons";

const MARKET_KEYS = ["forex", "metals", "indices", "energies", "crypto", "stocks"] as const;
const ICONS = [IconBarChart, IconTrendingUp, IconLayers, IconActivity, IconGlobe, IconMonitor];
const HREFS = ["/markets/forex", "/markets/metals", "/markets/indices", "/markets/energies", "/markets/crypto", "/markets/stocks"];
const INSTRUMENTS = [
  "EUR/USD · GBP/USD · USD/JPY · AUD/USD",
  "XAUUSD · XAGUSD",
  "US30 · NASDAQ · S&P500 · DAX40",
  "Brent Crude · WTI Crude",
  "BTC · ETH · LTC · XRP",
  "Apple · Tesla · Amazon · Google",
];
const SPREADS = ["From 0.0 pips", "From 25 pts", "From 1 pt", "From 3 pts", "From 50 pts", "Market"];
const LEVS   = ["1:500", "1:200", "1:100", "1:100", "1:10", "1:10"];

export default function MarketsSection() {
  const t = useTranslations("home.markets");

  return (
    <section className="py-20 lg:py-28 bg-[#F5F7FA]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12">
          <div>
            <div className="text-[11px] font-semibold text-[#00CC44] uppercase tracking-widest mb-4">{t("label")}</div>
            <h2 className="text-4xl font-extrabold text-[#111827]">{t("title")}</h2>
          </div>
          <Link href="/markets" className="text-sm font-semibold text-[#111827] hover:text-[#00CC44] transition-colors flex items-center gap-1 flex-shrink-0">
            {t("viewAll")}
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 18l6-6-6-6" /></svg>
          </Link>
        </div>

        {/* Grid */}
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {MARKET_KEYS.map((key, i) => {
            const Icon = ICONS[i];
            return (
              <Link key={key} href={HREFS[i]}
                className="group bg-white border border-gray-100 rounded-2xl p-6 hover:border-[#111827]/15 hover:shadow-lg transition-all duration-300">
                <div className="flex items-start justify-between mb-5">
                  <div className="w-10 h-10 rounded-xl border border-gray-200 bg-[#F5F7FA] text-gray-500 flex items-center justify-center group-hover:border-[#111827]/20 group-hover:text-[#111827] transition-colors">
                    <Icon className="w-5 h-5" />
                  </div>
                  <span className="text-[11px] text-gray-400 border border-gray-100 px-2.5 py-1 rounded-full">{t(`${key}_tag`)}</span>
                </div>
                <h3 className="text-xl font-bold text-[#111827] mb-1.5 group-hover:text-[#00AA38] transition-colors">{t(`${key}_label`)}</h3>
                <p className="text-[13px] text-gray-400 mb-5 leading-relaxed">{INSTRUMENTS[i]}</p>
                <div className="flex gap-2">
                  <span className="text-[11px] font-semibold text-[#00AA38] bg-[#00CC44]/8 border border-[#00CC44]/12 px-2.5 py-1 rounded-full">{SPREADS[i]}</span>
                  <span className="text-[11px] font-medium text-gray-500 bg-gray-100 px-2.5 py-1 rounded-full">{t("lev")} {LEVS[i]}</span>
                </div>
              </Link>
            );
          })}
        </div>
      </div>
    </section>
  );
}
